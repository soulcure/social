...
function generateTTF() {
    return src(["../social/assets/icon-font/**/*.svg"])
        .pipe(
            rename(path => {
                if (path.dirname != ".") {
...
`;
    const outFile = "../social/lib/icon_font.dart";
...
async function generateSVGListDartFile() {
    ...
    `;
        const outFile = "../social/lib/svg_icons.dart";
        writeFileSync(outFile, content);
    }

    const files = walk
        .sync("../social/assets", { no_recurse: false })
