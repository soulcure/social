...
class TextLine extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (node.hasEmbed) {
      ...
      // 修改，添加embedSize参数
      final width = (embed.value.data['width'] as num).toDouble();
      final height = (embed.value.data['height'] as num).toDouble();
      final size =
          (width != null && height != null) ? Size(width, height) : null;
      return EmbedProxy(embedSize: size, child: embedBuilder(context, embed));
    }
  ...
  TextStyle _getInlineTextStyle(NotusStyle style, ZefyrThemeData theme) {
    ...
    if (style.contains(NotusAttribute.at)) {
      result = _mergeTextStyleWithDecoration(result, theme.at);
    }
    if (style.contains(NotusAttribute.channel)) {
      result = _mergeTextStyleWithDecoration(result, theme.channel);
    }
    return result;
  }
