...
mixin RawEditorStateTextInputClientMixin on EditorState
  ...
  void updateEditingValue(TextEditingValue value) {
    ...
    try {
      ...
      final content = widget.controller.document.toPlainText();
      final isChinese = value.composing.start != value.composing.end;
      final shouldWrap = diff.start - 1 >= 0 &&
          diff.start - 1 <= content.length &&
          content[diff.start - 1] == '\uFFFC';
      if (isChinese && shouldWrap) {
        _lastKnownRemoteTextEditingValue = effectiveLastKnownValue;
        return;
      }
      ;
      _remoteValueChanged(
