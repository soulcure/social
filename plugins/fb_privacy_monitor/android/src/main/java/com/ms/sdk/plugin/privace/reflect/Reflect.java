...
public class Reflect {
    ...
    public static final String TAG = Reflect.class.getSimpleName();
    ...
    /**从指定的类加载起寻找类,并构建反射工具类
     * @param name        类的全名
     * @param shouldInitialize  是否初始化静态区域
     * @param classLoader 需要构建工具类的类的类加载器 loaded.
     * @return 反射工具类
     * @throws ReflectException 如果反射出现意外
     * @see #on(Class)
     */
    public static Reflect on(String name, boolean shouldInitialize, ClassLoader classLoader) throws ReflectException {
        return on(forName(name, shouldInitialize, classLoader));
    }
     ...
     */
    private static Class<?> forName(String name, boolean shouldInitialize, ClassLoader classLoader) throws ReflectException {
        try {
            return Class.forName(name, shouldInitialize, classLoader);
        } catch (Exception e) {
            throw new ReflectException(e);
        }
    }
