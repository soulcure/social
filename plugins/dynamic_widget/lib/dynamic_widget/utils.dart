...
BoxDecoration? parseDecoration(Map<String, dynamic>? map, Color? color) {
  if (map == null) return null;
  BorderRadiusGeometry radius;
  Border border;

  final String? borderString = map['border'];
  final String? radiusString = map['radius'];

  if (borderString.hasValue) {
    Color color = parseHexColor(map['borderColor']) ?? Colors.black;
    var borders = borderString!.split(",");
    border = Border(
      top: BorderSide(width: double.parse(borders[0]), color: color),
      left: BorderSide(width: double.parse(borders[1]), color: color),
      right: BorderSide(width: double.parse(borders[2]), color: color),
      bottom: BorderSide(width: double.parse(borders[3]), color: color),
    );
  } else {
    border = Border();
  }
  ...
  // 没有radius，就不能传
  if (radiusString.hasValue) {
    final radius = BorderRadius.circular(double.parse(radiusString!));
    return BoxDecoration(color: color, border: border, borderRadius: radius);
  }
