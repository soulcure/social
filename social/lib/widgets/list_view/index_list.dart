...
class IndexListView extends BoxScrollView {
  ...
  IndexListView.builder({
    ...
    indexController?._initialPadding = indexPadding ?? EdgeInsets.zero;
  }
...
class IndexController extends ScrollController {
  ...
  @override
  IndexScrollPosition get position =>
      hasClients ? super.position as IndexScrollPosition : null;
  ...
  void _calculateIndex(int first, int last, double screenOff) {
    if (position == null || _indexListener == null) return;
...
class IndexScrollPosition extends ScrollPositionWithSingleContext {
  ...
  double getEndPixel() {
    return maxScrollExtent ?? _indexMap?.values?.last ?? 0.0;
  }
