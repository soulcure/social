...
import 'package:im/pages/logging/ws_log_view.dart';
...
class LoggerPage extends StatelessWidget {
  ...
  static void log(Object message, [Object detail]) {
    if (Get.isLogEnable) _Log.add(_Type.log, message, detail);
  }

  /// Record debug information
  static void debug(Object message, [Object detail]) {
    if (Get.isLogEnable) _Log.add(_Type.debug, message, detail);
  }

  /// Record warnning information
  static void warn(Object message, [Object detail]) {
    if (Get.isLogEnable) _Log.add(_Type.warn, message, detail);
  }

  /// Record error information
  static void error(Object message, [Object detail]) {
    if (Get.isLogEnable) _Log.add(_Type.error, message, detail);
  }

  /// Start recording time
  static void time(Object key) {
    assert(key != null);
    if (Get.isLogEnable) _Log.time(key);
  }

  /// End of record time
  static void endTime(Object key) {
    assert(key != null);
    if (Get.isLogEnable) _Log.endTime(key);
  }
  ...
  static void net(String api,
    ...
    assert(api != null);
    if (Get.isLogEnable) _Net.request(api, type, status, data);
  }
  ...
  static void endNet(String api,
    ...
    assert(api != null);
    if (Get.isLogEnable) _Net.response(api, status, data, headers, type);
  }

  static void ws(WsLogType type, Map data) {
    if (!Get.isLogEnable) return;

    _Log.wsLogs.add(WsLog(
      time: DateTime.now(),
      data: data,
      type: type,
    ));
...
class WsLog {
  final Map data;
  final WsLogType type;
  final DateTime time;

  const WsLog({this.data, this.type, this.time});
}

class _Log {
  static final List<_Log> list = [];
  static final RxList<WsLog> wsLogs = RxList();
  ...
  static void _clearWhenTooMuch() {
    if (list.length > LoggerPage.config.maxLimit) {
      list.removeRange(0, (LoggerPage.config.maxLimit * 0.2).ceil());
    }
  }
...
class _Net extends ChangeNotifier {
  ...
  String toString() {
    ...
    sb.writeln("Request: ${jsonEncode(req)}");
    sb.writeln("Response: ${jsonEncode(res)}");
    return sb.toString();
  }
  ...
  static void request(String api, String type, int status, Object data) {
    ...
    _clearWhenTooMuch();
    length.value++;
    // if (LoggerPage.config.printNet) {
    //   debugPrint(
    //       "${_printNames[4]} ${type == null ? '' : '$type: '}${net.api}");
    // }
  }
